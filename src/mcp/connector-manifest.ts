...
export const GOVERNED_CONNECTOR_MANIFEST_VERSION = 4 as const;
...
export const CHRYPCK_GOVERNED_CONNECTOR_MANIFEST: ChryPckGovernedConnectorManifest = Object.freeze({
  ...
  connector: Object.freeze({
    ...
    description: "Policy-enforced governed repository development. ChryPck is designed to be the model's sole repository capability: it builds an exhaustive hidden Repository Model; requires a bounded host-LLM Semantic Atlas bootstrap on uncached repository state; projects structural/semantic orientation, compressed dependency/runtime/symbol/state diagnostics, one canonical bounded evidence-backed Trace engine, a certified Patch Corridor and Context Pack; executes coherent bounded cross-file patches; performs Change Propagation and broad validation; and returns authoritative terminal evidence.",
  ...
  workflow: Object.freeze({
    ...
    summary: "Use ChryPck as a staged repository workflow. Complete any mandatory semantic bootstrap first, orient with the Structural and Semantic Atlases, narrow with compressed diagnostics, use the single canonical trace mode for bounded evidence-backed runtime/dependency/handler/state/effect paths, expand only server-certified Context Pack segments, synthesize one coherent patch across all certified touch points, execute only with current user mutation authorization, and verify through the authoritative result.",
    diagnosticSurfaces: Object.freeze([
      ...
      Object.freeze({ id: "patch-corridor", purpose: "Certifies the files and symbols the model may change for the admitted objective." }),
    ...
    phases: Object.freeze([
      ...
      Object.freeze({ id: "general-analysis", toolNames: Object.freeze(["chrypck_plan"] as const), guidance: "Use dependency graph/watershed, symbol families, effect atlas, integration surfaces, runtime signals, state namespaces, native contracts, runtime probes and Patch Corridor to narrow the objective. Do not mistake omitted compressed evidence for absence." }),
      Object.freeze({ id: "focused-analysis", toolNames: Object.freeze(["chrypck_plan"] as const), guidance: "Use analysis.kind='trace' for a concrete bug/runtime/dependency/state/effect question. Trace is the former BEFT engine promoted to the only trace mode: bounded, corridor-aware, evidence-backed and certifiable. Let it auto-resolve an evidence-supported source when appropriate, or provide sourceSymbol/targetEffect plus hop/branch/file/symbol bounds when known." }),
      Object.freeze({ id: "certified-context", toolNames: Object.freeze(["chrypck_context"] as const), guidance: "Use chrypck_context only after orientation and objective-local analysis identify the exact touch points. Read the certified Context Pack index and only server-issued source segments needed to understand implementation or author the patch. Never substitute arbitrary path reads." }),
      Object.freeze({ id: "cross-cutting-patch", toolNames: Object.freeze(["chrypck_plan", "chrypck_context"] as const), guidance: "Synthesize the complete fix across the full certified runtime/dependency path. Use Semantic Atlas for intent, Structural Atlas for location, diagnostics/trace for relationships and causal evidence, and Context Pack for exact implementation. Include every necessary certified touch point in one coherent bounded authoring intent." }),
      Object.freeze({ id: "execute", toolNames: Object.freeze(["chrypck_execute"] as const), guidance: "Only after the current user turn authorizes mutation, submit the coherent typed authoring_intent or explicitly approve a server-issued architecture plan. ChryPck stages, propagates, validates and atomically publishes it; semantic/trace evidence never expands mutation authority." }),
      Object.freeze({ id: "verify", toolNames: Object.freeze(["chrypck_result"] as const), guidance: "Always read chrypck_result after execution or failure and report only authoritative terminal, propagation, validation and publication evidence returned by ChryPck." })
    ]),
    patchStrategy: "Prefer one coherent cross-cutting patch that covers all certified touch points along the relevant runtime/dependency path. ChryPck is specifically designed to make multi-file fixes tractable by exposing repository-wide structural and semantic orientation first, compressed objective-local relationships and canonical bounded Trace second, and exact source only where justified.",
