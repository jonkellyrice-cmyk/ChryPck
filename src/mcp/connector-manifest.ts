...
export const GOVERNED_CONNECTOR_MANIFEST_VERSION = 7 as const;
...
export const CHRYPCK_GOVERNED_CONNECTOR_MANIFEST: ChryPckGovernedConnectorManifest = Object.freeze({
  ...
  workflow: Object.freeze({
    ...
    phases: Object.freeze([
      ...
      Object.freeze({ id: "focused-analysis", toolNames: Object.freeze(["chrypck_plan"] as const), guidance: "Use analysis.kind='trace' for a concrete bug/runtime/dependency/state/effect question. Effect / Runtime Atlas maps possible behavior; Trace is the only trace mode and certifies one bounded evidence-backed causal route. Let Trace use an Atlas-supported entrypoint, or provide sourceSymbol/targetEffect plus hop/branch/file/symbol bounds. A useful Trace ends with a persisted artifact and certificate and normally returns create_normal_plan_with_trace_handoff." }),
...
function validateManifest(manifest: ChryPckGovernedConnectorManifest): void {
  ...
  if (!surfaceIds.has("semantic-atlas") || !surfaceIds.has("semantic-coverage-ledger") || !surfaceIds.has("analysis-lineage") || !surfaceIds.has("contract-map") || !surfaceIds.has("effect-runtime-atlas")) {
    throw new Error("ChryPck governed connector manifest must publish Semantic Atlas, Contract Map, Effect / Runtime Atlas and certified analysis-lineage surfaces.");
  }
