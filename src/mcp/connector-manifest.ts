...
export const GOVERNED_CONNECTOR_MANIFEST_VERSION = 6 as const;
...
export const CHRYPCK_GOVERNED_CONNECTOR_MANIFEST: ChryPckGovernedConnectorManifest = Object.freeze({
  ...
  workflow: Object.freeze({
    ...
    phases: Object.freeze([
      ...
      Object.freeze({ id: "general-analysis", toolNames: Object.freeze(["chrypck_plan"] as const), guidance: "Use dependency graph/watershed, symbol families, effect atlas, integration surfaces, runtime signals, state namespaces, native contracts, the reconciled Contract Map, runtime probes and the initial Patch Corridor to narrow the objective. Treat Contract Map conflicts as blockers on affected work; Contract Map evidence never grants mutation scope. Do not mistake omitted compressed evidence for absence." }),
...
function validateManifest(manifest: ChryPckGovernedConnectorManifest): void {
  ...
  if (!surfaceIds.has("semantic-atlas") || !surfaceIds.has("semantic-coverage-ledger") || !surfaceIds.has("analysis-lineage") || !surfaceIds.has("contract-map")) {
    throw new Error("ChryPck governed connector manifest must publish Semantic Atlas, Contract Map and certified analysis-lineage surfaces.");
  }
