...
export const GOVERNED_CONNECTOR_MANIFEST_VERSION = 9 as const;
...
export const CHRYPCK_GOVERNED_CONNECTOR_MANIFEST: ChryPckGovernedConnectorManifest = Object.freeze({
  ...
  capabilities: Object.freeze([
    Object.freeze({ toolName: "chrypck_plan", label: "Orient, analyze and plan governed repository work", description: "Start all governed repository work. ChryPck builds global deterministic structure immediately and lazily requests at most one objective-local semantic region when navigation reaches an unmapped boundary. Global Semantic Atlas completion is never required; OBJECTIVE_SUFFICIENT coverage permits bounded work. Choose Trace for causal runtime evidence or Dataflow Slice for value provenance/influence. Certified analysis_handoff informs a distinct normal plan without granting source or mutation authority; trace_handoff remains compatible.", effect: "read", approval: "automatic", returns: "Either one bounded objective-local semantic expansion; a focused Trace or Dataflow Slice artifact with next action; or a normal plan with structural orientation, sparse incremental Semantic Atlas and global/objective coverage, diagnostics, accepted analysis lineage, independently certified Patch Corridor/Context Pack, and next permitted action.", errorBehavior: "Returns structured semantic-expansion continuation/restart, policy, focused-analysis-certification, analysis-lineage, timeout or transport evidence. Fail-closed outcomes are authoritative and must not be bypassed.", programmaticEligible: false }),
    Object.freeze({ toolName: "chrypck_context", label: "Read certified repository context", description: "Read only server-certified Context Pack evidence for an existing READY normal-plan run with objective-sufficient semantic coverage and any focused-analysis handoff. Focused-analysis runs do not expose arbitrary exact source; use analysis_handoff to create a fresh normal plan first.", effect: "read", approval: "automatic", returns: "The certified context index or one bounded server-issued exact-source context segment together with the normal run's next permitted action.", errorBehavior: "Returns structured run, semantic-expansion, policy or transport failure evidence. Arbitrary repository paths and analysis-run source requests are never accepted as fallback.", programmaticEligible: false }),
    ...
    Object.freeze({ toolName: "chrypck_result", label: "Read governed repository result", description: "Read the authoritative bounded state and outcome of an existing ChryPck run after lazy semantic orientation, planning, focused analysis, analysis handoff, execution, timeout or failure.", effect: "read", approval: "automatic", returns: "Bounded run state, project profile, incremental Semantic Atlas with global/objective coverage, persisted Trace or Dataflow Slice artifact/certificate when applicable, accepted analysis_handoff lineage, diagnostics, propagation and validation evidence, telemetry, terminal status and publication state.", errorBehavior: "Returns structured run or transport failure evidence. Missing or failed run state must be reported rather than inferred.", programmaticEligible: false })
  ])
});
