...
function buildMcpServer(): McpServer {
  const server = new McpServer({ name: "chrypck", version: "0.2.0" });
  server.registerTool("chrypck_plan", {
    title: "Plan Governed Change",
    description: "Create a native governed run, snapshot the repository, and return certified diagnostics/Patch Corridor evidence.",
    inputSchema: z.object({ repository: z.string().min(3), objective: z.string().trim().min(1), base_ref: z.string().trim().min(1).optional() }),
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }
  }, async input => toolBoundary(() => nativeService.plan(input)));

  server.registerTool("chrypck_context", {
    title: "Read Certified Context",
    description: "Return only the certified Context Pack for a READY native run; arbitrary repository paths are not accepted.",
  ...
  server.registerTool("chrypck_execute", {
    title: "Execute Governed Change",
    description: "Compile typed edits against certified context, stage atomically, validate, and publish through the repository adapter.",
    inputSchema: z.object({
      run_id: z.string().min(1),
      authoring_intent: z.object({ id: z.string().trim().min(1), objective: z.string().trim().min(1), edits: z.array(authoringEditSchema).min(1) }),
  ...
  server.registerTool("chrypck_result", {
    title: "Read Governed Result",
    description: "Return native run state, bounded artifacts, telemetry, and terminal evidence.",
...
function reject(res: ServerResponse, status: number, message: string): void {
  res.writeHead(status, { "content-type": "application/json; charset=utf-8" }); res.end(JSON.stringify({ error: message }));
}
function authorizeHttp(req: IncomingMessage, res: ServerResponse): boolean {
  if (config.publicHost) {
    const incomingHost = String(req.headers.host ?? "").toLowerCase(), hostname = incomingHost.split(":")[0] ?? incomingHost;
    if (incomingHost !== config.publicHost && hostname !== config.publicHost) { reject(res, 403, "Host is not allowed."); return false; }
  }
  const origin = String(req.headers.origin ?? "").trim();
  if (origin && config.allowedOrigins.size > 0 && !config.allowedOrigins.has(origin)) { reject(res, 403, "Origin is not allowed."); return false; }
  if (config.mcpBearerToken) {
    const authorization = String(req.headers.authorization ?? ""), expected = `Bearer ${config.mcpBearerToken}`;
    if (!constantTimeTokenMatch(authorization, expected)) { reject(res, 401, "Bearer authorization is required."); return false; }
  }
  return true;
}
...
const httpServer = createServer((req, res) => {
  ...
  if (requestUrl.pathname === "/healthz" && req.method === "GET") {
    ...
    res.end(JSON.stringify({ ok: true, service: "chrypck", version: "0.2.0", execution: "native" })); return;
  }
  if (requestUrl.pathname !== "/mcp") { reject(res, 404, "Not found."); return; }
  if (!authorizeHttp(req, res)) return;
  void nodeHandler(req, res);
});

httpServer.listen(config.port, config.host, () => {
  console.error(`ChryPck listening on http://${config.host}:${config.port}/mcp`);
  if (!config.mcpBearerToken && config.host !== "127.0.0.1" && config.host !== "localhost" && config.host !== "::1") console.error("WARNING: MCP_BEARER_TOKEN is not configured on a non-loopback bind.");
});
