...
import type { NativeRunArtifacts } from "./run-artifacts.js";
...
export interface NativeToolchainRun {
  ...
  readonly artifacts: NativeRunArtifacts;
  state: RunState;
...
export class RunStore {
  ...
  private requestKey(repository: string, fingerprint: string): string {
  ...
  get(runId: string): NativeToolchainRun | null {
  ...
  snapshot(): readonly NativeToolchainRun[] {
