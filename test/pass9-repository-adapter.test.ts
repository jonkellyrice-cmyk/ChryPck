...
test("GitHub adapter exposes hydrated immutable snapshots without leaking transport", async()=>{const transport=fakeTransport(),adapter=new GitHubRepositoryAdapter(transport);const snapshot=await adapter.snapshot("owner/repo","main");assert.equal(snapshot.commitSha,"a".repeat(40));assert.equal(snapshot.files.find(file=>file.path==="src/a.ts")?.text,"export const a = 1;");assert.equal(snapshot.files.find(file=>file.path==="image.png")?.text,undefined);assert.deepEqual(transport.calls,["resolve:main","list","read:src/a.ts"]);});
