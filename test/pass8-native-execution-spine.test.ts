...
import { createValidationCommandPlan } from "../src/validation/command-plan.js";
...
test("native spine reaches terminal success without workflow telemetry", async()=>{const o=new NativeOrchestrator(),run=o.admitRequest({repository:"owner/repo",request,requestPath:"request.json"});o.bindRequestCommit(run.runId,"a".repeat(40));const result=await executeNativeRun(o,{runId:run.runId,model,intent:{id:"edit",objective:"Change provider",edits:[{type:"replace_exact",path:"src/provider.ts",search:"return 1",replace:"return 2"}]},structuralValidators:[createStructuralValidator({maxFileBytes:4096})],commandPlan:createValidationCommandPlan([]),sandboxRunner:sandbox,committer:{async commit(staged){return{commitSha:"b".repeat(40),changedPaths:staged.changedPaths,patchFingerprint:staged.patchFingerprint}}}});assert.equal(result.run.state,"SUCCEEDED");assert.equal(result.run.resultCommitSha,"b".repeat(40));assert.equal(result.run.artifacts.validation?.passed,true);assert.ok(result.run.telemetry.snapshot().every(event=>event.source==="native"));});

test("native spine fails closed on objective mismatch",async()=>{const o=new NativeOrchestrator(),run=o.admitRequest({repository:"owner/repo",request:{...request,id:"mismatch"},requestPath:"request.json"});const result=await executeNativeRun(o,{runId:run.runId,model,intent:{id:"edit",objective:"Broaden objective",edits:[{type:"replace_exact",path:"src/provider.ts",search:"return 1",replace:"return 2"}]},structuralValidators:[],commandPlan:createValidationCommandPlan([]),sandboxRunner:sandbox,committer:{async commit(){throw new Error("must not commit")}}});assert.equal(result.run.state,"FAILED");assert.equal(result.run.artifacts.failure?.failed_stage,"execution");});
